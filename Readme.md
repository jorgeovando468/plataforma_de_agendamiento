...
- **Errores `connect ECONNREFUSED` o `ETIMEDOUT` al iniciar el backend**: indican que el backend no puede conectarse a MySQL (contenedor caído, puerto incorrecto o arranque lento). Solución: 1) verifica que `consultorio-db` esté sano (`docker compose ps` debe mostrar `healthy`); 2) si lo recreaste, espera a que termine de inicializarse o reinicia con `docker compose down -v && docker compose up -d db backend frontend`; 3) confirma que `DB_HOST=db` y `DB_PORT` coincide con el puerto interno (3306) o el que uses fuera de Docker; 4) si tu servidor tarda más en arrancar, eleva los tiempos con `DB_WAIT_RETRIES=30` y `DB_WAIT_DELAY_MS=5000` en `Backend/.env`.
